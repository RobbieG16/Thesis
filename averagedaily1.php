...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        ...
        $checkQuery = "SELECT * FROM avgsensor1 WHERE date = '$latestTime'";  // Check based on the latest timestamp
        $checkResult = $conn->query($checkQuery);

        if ($checkResult->num_rows > 0) {
            $updateQuery = "UPDATE avgsensor1 
            ...
            $insertQuery = "INSERT INTO avgsensor1 
