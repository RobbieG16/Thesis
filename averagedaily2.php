...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        ...
        $checkQuery = "SELECT * FROM avgsensor2 WHERE date = '$latestTime'";  // Check based on the latest timestamp
        $checkResult = $conn->query($checkQuery);

        if ($checkResult->num_rows > 0) {
            $updateQuery = "UPDATE avgsensor2 
            ...
            $insertQuery = "INSERT INTO avgsensor2 
